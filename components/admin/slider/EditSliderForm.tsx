...
import { updateSlider } from "@/app/actions/sliderAction";
import toast from "react-hot-toast";

const EditSliderForm = ({ slider }: { slider: any }) => {
    const router = useRouter();
    const [isPending, startTransition] = useTransition();
    const [preview, setPreview] = useState<string | null>(slider.image || null);
    ...
    const clientAction = async (formData: FormData) => {
        ...
        startTransition(async () => {
            const result = await updateSlider(slider._id.toString(), formData);
            if (result.success) {
    ...
    return (
        <form action={clientAction} className="p-8 md:p-12 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">
                        Slider Banner Image (Optional to update)
                    </label>
